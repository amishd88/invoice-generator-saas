...
DECLARE
    view_def TEXT;
BEGIN
    -- Check if user_sales_report exists and uses EXTRACT
    ...
    -- Check if user_outstanding_invoices exists and uses EXTRACT
    SELECT pg_get_viewdef('user_outstanding_invoices', true) INTO view_def;
    IF view_def ILIKE '%extract%due_date%' THEN
        RAISE NOTICE 'Recreating user_outstanding_invoices view';
        -- You'll need to adapt this to match your actual view definition
        DROP VIEW IF EXISTS user_outstanding_invoices;
        
        -- Create a placeholder view - replace with actual definition
